...
struct http2_stream {
	...
	int in_ready_list;                /* Flag: is stream in ready_streams list? */
	...
	int body_buffer_len;
	int content_length;           /* Cached content-length from headers (-1: not set) */
...
struct http2_ctx {
	...
	struct list_head ready_streams;   /* Streams that just became ready */
...
/* Check content-length using cached value (avoids repeated header scanning) */
...
{
	/* Use cached content_length from headers, populated when HEADERS frame completes */
	if (stream->content_length < 0) {
		return 0;
	}

	if (body_len > stream->content_length || (end_stream && body_len != stream->content_length)) {
...
static struct http2_stream *_http2_create_stream(struct http2_ctx *ctx, uint32_t stream_id)
{
	...
	stream->content_length = -1;  /* Initialize: not yet cached */
...
/* Mark stream as ready for processing (became readable).
   Caller must hold ctx->mutex */
static int _http2_mark_stream_ready(struct http2_stream *stream)
{
	struct http2_ctx *ctx;
	
	if (!stream || stream->in_ready_list) {
		return 0;  /* Already in list or invalid */
	}
	...
	/* Move to ready_streams list for fast retrieval in poll */
	if (!list_empty(&stream->node)) {
		list_move_tail(&stream->node, &ctx->ready_streams);
		stream->in_ready_list = 1;
		return 1;
	}
...
static int _http2_process_data_frame(struct http2_ctx *ctx, int stream_id, const uint8_t *data, int len, uint8_t flags)
{
	...
	if (len > 0) {
		...
		/* If stream is now readable (has data or ended), mark it ready for quick poll retrieval */
	int became_readable = 0;
	if (stream->end_stream_received && stream->body_buffer_len > 0) {
		/* Stream ended and has body data */
		became_readable = _http2_mark_stream_ready(stream);
	} else if (len > 0 && !stream->end_stream_received) {
		/* New data arrived, stream may have become readable */
		became_readable = _http2_mark_stream_ready(stream);
	}
	(void)became_readable;  /* May use for future notifications */
										...
										uint8_t flags, uint8_t frame_type)
{
	...
	/* Cache content-length after HEADERS frame is complete to avoid repeated header scans */
	if ((flags & HTTP2_FLAG_END_HEADERS) != 0) {
											 ...
											 int *count, int check_writable)
{
	...
	/* First, process ready_streams (streams that just became ready) */
	list_for_each_entry_safe(stream, tmp, &ctx->ready_streams, node)
	{
		...
		items[*count].readable = 1;
		items[*count].writable = 
			(stream->state == HTTP2_STREAM_OPEN || stream->state == HTTP2_STREAM_HALF_CLOSED_REMOTE) ? 1 : 0;
		stream->in_ready_list = 0;
		list_move_tail(&stream->node, &ctx->streams);  /* Move back to main list */
		(*count)++;
	}
	
	/* If still have room, scan remaining streams for late-arrival readiness changes */
	list_for_each_entry_safe(stream, tmp, &ctx->streams, node)
	{
...
int http2_stream_write_body(struct http2_stream *stream, const uint8_t *data, int len, int end_stream)
{
	...
	if (len == 0 && end_stream) {
		...
		int ret = _http2_send_data_frame(ctx, stream->stream_id, data, 0, 1);
		if (ret < 0) {
			pthread_mutex_unlock(&ctx->mutex);
			return ret;
		}
